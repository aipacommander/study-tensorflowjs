import { Paddle } from '@/core/game/paddle'
import { Ball } from '@/core/game/ball'
import { DIRECTION, rounds, colors } from '@/core/game/types'
...
export class Game {
	...
	update() {
		if (!this.over) {
			...
			if (this.ball.x >= this.canvas.width - this.ball.width) this._resetTurn.call(this, this.player, this.paddle);
		...
		if (this.player.score === rounds[this.round]) {
			...
			this.over = true
			const self = this
			setTimeout(function () { self.endGameMenu('Winner!') }, 1000)
		...
		// Check to see if the paddle/AI has won the round.
			...
			this.over = true
			const self = this
			setTimeout(function () { self.endGameMenu('Game Over!') }, 1000)
		}
	}
	...
	listen() {
		...
		document.addEventListener('keydown', function (key) {
			...
			if (key.keyCode === 38 || key.keyCode === 87) self.player.move = DIRECTION.UP

			// Handle down arrow and s key events
			if (key.keyCode === 40 || key.keyCode === 83) self.player.move = DIRECTION.DOWN
		});

		// Stop the player from moving when there are no keys being pressed.
		document.addEventListener('keyup', function (key) { self.player.move = DIRECTION.IDLE; });
	}
